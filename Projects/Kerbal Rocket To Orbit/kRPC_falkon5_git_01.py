...
- kRPC 0.3.11 (https://forum.kerbalspaceprogram.com/index.php?/topic/62902-131-krpc-remote-procedure-call-server-v0311-6th-october-2017/)
...
def prepare_screen_message():
	...
	text.rect_transform.position = (-140,0)
	text.color = (1,1,1)
	text.size = 13
	return text
...
secondst_heading = 0
print("Second stage: pitch to 15, head to " + str(secondst_heading))
second_stage.auto_pilot.target_pitch_and_heading(15, secondst_heading)
rocket.auto_pilot.target_pitch_and_heading(15,secondst_heading)
...
time.sleep(2)
print("Second stage: pitch to 5, head to " + str(secondst_heading))
second_stage.auto_pilot.target_pitch_and_heading(5, secondst_heading)
...
crude_suicide_burn_factor = 3.2
crude_suicide_burn_height = crude_suicide_burn_factor * rocket.flight(srf_frame).speed
...
write_screen_message(text_panel, "Burn at: " + str(crude_suicide_burn_height) + " m")
print("Burn at: " + str(crude_suicide_burn_height) + " m")

while rocket.flight().mean_altitude > crude_suicide_burn_height:
	write_screen_message(text_panel, 'Suicide burn at ' + str(crude_suicide_burn_height) + " m")
	#print('Here we go!')
	interval_print("Alt: " + str(rocket.flight().mean_altitude) + " spd: " + str(rocket.flight(srf_frame).speed) + " Sit: " + str(rocket.situation))
...
while rocket.flight().mean_altitude > 1:
	...
	elif rocket.flight(srf_frame).speed < 10:
